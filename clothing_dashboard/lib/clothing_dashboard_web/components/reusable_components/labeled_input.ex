    ...
    def labeled_input(assigns) do
        ~H"""
            <div class={"relative w-full group #{@wrapper_classes}"}>
                <%= if @is_textarea do %>
                    <textarea  id={@id}  name={String.downcase(@name)}  required  class="peer w-full bg-transparent px-5 py-4 text-xl border rounded outline-none transition-shadow duration-300 hover:shadow-lg focus:shadow-lg valid:shadow-lg" />
                <% else %>
